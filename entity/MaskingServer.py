...
class MaskingServer():
    ...
    def generate_ramdom_matrix_T_block(self, block_size):
        self.T = generate_block_random_martic(int(self.size/block_size), block_size, "T")
    ...
    def generate_random_matrices_block(self, block_size):
        for i in range(2):
            self.random_matrices_1.append(generate_block_random_martic(int(self.size/block_size), block_size, "random_matrices_1_"+str(i)))
            self.random_matrices_2.append(
                generate_block_random_martic(int(self.size/block_size), block_size, "random_matrices_2_" + str(i)))
    ...
    def generate_orthogonal_matrices(self, D):
        ...
        self.DQ = np.dot(np.diag(D ** -1), self.Q)
        return self.Q
